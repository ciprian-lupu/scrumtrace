...
def fill_brief(tasks_html: str, shots_html: str, timeline_html: str) -> str:
    ...
    replacements = {
        ...
        "{{PAUSE_COUNT}}": "1 pause",
        "{{CONTEXT_BADGE}}": "",
        "{{STATUS_HTML}}": '<section id="processing"><h2>Processing status</h2><p>Synthetic sample with two confirmed tasks.</p></section>',
        "{{SUMMARY_HTML}}": '<section id="summary"><h2>Session summary</h2><p>Two implementation tasks supported by selected screenshot and video evidence.</p></section><section id="decisions"><h2>Decisions</h2><p>No decisions captured in this sample.</p></section><section id="actions"><h2>Next actions</h2><p>Review the two confirmed findings below.</p></section><section id="questions"><h2>Open questions</h2><p>No open questions captured in this sample.</p></section>',
        "{{LOCAL_OUTLINE_HTML}}": '<section id="local-outline" class="brief-panel"><h2>Local timeline <span class="muted">review-only</span></h2><p class="muted">Bounded extractive passages selected locally; confirm their meaning against the linked evidence.</p><article class="outline-section"><h3>t_media 03:04–03:08</h3><p class="clip-meta">Visual and transcript evidence</p><blockquote><span class="spk">presenter</span><span class="when">t_media 03:04–03:07</span>this does nothing, it should store the athlete</blockquote><p><a href="media/task-02/clip.mp4">Open selected clip</a> · <a href="shots/001.annotated.png">Open Shot</a></p></article></section>',
        "{{SPEAKERS_HTML}}": "",
        "{{DOWNLOADS_HTML}}": '<section id="exports"><h2>Use this session</h2><a href="AGENT_CONTEXT.md">Agent context</a> · <a href="AGENT_PROMPT.txt">Agent instructions</a></section>',
        "{{REVIEW_OPEN}}": "",

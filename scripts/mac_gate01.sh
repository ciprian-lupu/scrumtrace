...
LSUI="$(/usr/libexec/PlistBuddy -c 'Print :LSUIElement' "$APP/Contents/Info.plist" 2>/dev/null || echo false)"
if [[ "$LSUI" != "true" && "$LSUI" != "1" ]]; then
  echo "LSUIElement must be true for agent hotkeys (got: $LSUI)" >&2
  exit 1
fi
echo "bundle_id=$ACTUAL_ID"
...
ditto "$APP" "$STABLE"
sign_app "$STABLE"
# Gate 0/1 require an unsandboxed agent; refuse if codesign embeds sandbox=true.
if codesign -d --entitlements :- "$STABLE" 2>/dev/null | grep -q 'com.apple.security.app-sandbox</key>[[:space:]]*<true'; then
  echo "codesign reports App Sandbox enabled — refuse Gate 0/1" >&2
  exit 1
fi
if [[ -f "$ENTITLEMENTS" ]] && /usr/libexec/PlistBuddy -c 'Print :com.apple.security.app-sandbox' "$ENTITLEMENTS" 2>/dev/null | grep -qi true; then
  echo "App Sandbox entitlement is enabled in $ENTITLEMENTS" >&2
  exit 1
fi

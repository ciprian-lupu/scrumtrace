...
Settings (⌘, or menu → Settings → Settings Window…) is a section of the ScrumTrace window with **Speech**, **Capture**, **Logs**, **Permissions**, **AI**, and **General**. Capture shows the shipped 3840×2160 / 4 fps / 16 Mbps (24 Mbps cap) budget, plus pointer and microphone toggles. Start recording opens a selection overlay like macOS screen recording (lasting box, move, resize, then Record or Return on that display). While recording, the floating HUD can be dragged anywhere, resized from its edges (the whole pill scales), and minified to the dot and clock with its chevron or a double-click; it remembers its place and size for the next recording and always starts expanded. The HUD and the Shot note window are hidden from screen sharing (Google Meet, Teams, Zoom) and from any other display capture; only you see them.

If ScrumTrace is quit, killed or reinstalled while a recording is still being processed (transcribing, slicing, evaluating or synthesizing), the next launch finishes that recording by itself; the menu's **Quit** asks first while processing is running. A crash during recording or a run that already failed offline still needs **Retry Analysis**. `scripts/mac_gate01.sh` refuses to replace the app while a recording is being processed unless `SCRUMTRACE_FORCE_INSTALL=1` is set. Logs can export a diagnostic bundle (no `archive/`).
...
- **Recordings** — every session, newest first, with date, context / product, recorded time, status, Shots, tasks and pack size. Search (⌘F) matches the session id, date, context and product names only, never transcripts, Shot notes or task titles; Escape clears it. Filter by status or context. The detail pane shows processing stages, upload consent, export files and Shot thumbnails from `export/shots/`. Actions: Reveal export/ (⌘R), Open in Claude, Open export in Codex, Open archive in Codex…, Open brief, Copy export path, Retry analysis, Review speakers…, Reveal archive… (after confirmation) and Delete… (Delete key; asks first, removes the recording and its Codex workspaces). Session-changing actions wait while a recording, analysis or archive copy runs. Drag a row to drop its `export/` folder into Cursor, Terminal or Finder; only `export/` is offered. A session whose manifest cannot be read is listed as **Unreadable manifest** with Reveal folder… and Delete… only.
...
Give the agent **`export/`** only by default. Never drop the session root or `archive/` (master `session.mp4`, `audio.wav`, full transcript, raw events) into an ordinary agent handoff. Paths inside the pack are relative to that folder (`shots/…`, `media/…`).

**Open export in Codex** in Recordings/Overview and **Open last export in Codex** in the menu follow **Settings → AI → Local coding agents → Open export in Codex**:

- **Codex app** (default): the first open asks for a descriptive **project name**, for example **Import Flow Review**, with the recorded context and date as a suggested default. The project contains a copy of the validated exported materials. The prepared prompt asks for the simple task title **Analyze export** and stays in the composer until you send it. The app must be installed and opened once on this Mac; no separate CLI installation is needed.
...
For a complete capture-quality or full-meeting review, choose **Open archive in Codex…** in Recordings, Overview, or the menu. It uses the **same desktop project for that recording**, adding a `source/` snapshot containing the canonical manifest, all available `archive/` files and the corresponding `export/`, if any. The task prompt requests **Analyze archive**, with the original media as evidence. Archive-first and export-first opens resolve to the same project; no export is required to diagnose an unprocessed recording. A single dialog covers the archive handoff and, on first open, the project name. ScrumTrace blocks capture and session changes during the background copy.

After adding the archive, `source/` stays accessible to later tasks in that project. **Analyze export** asks the agent to use only the exported evidence; it is not a filesystem access restriction. Opening the draft does not submit it, but Codex can send analyzed content to its model service when you submit the prompt. The archive action always uses the desktop app; the export Terminal preference remains unchanged.
...
The project lives under the recording's `codex-workspaces/` folder. Its binding is saved outside the reference snapshots: changing the recorded context or switching export/archive cannot create another project. Reopening refreshes only the chosen `export/` or `source/` snapshot, preserving notes outside both. Existing PR #11 workspaces are reused **without moving their paths**; rename their displayed project name in Codex, since the documented deep link has no project-renaming parameter. Deleting the recording also deletes its workspaces and notes. Transfers and Analyze a copy exclude both current and legacy derived Codex workspaces.

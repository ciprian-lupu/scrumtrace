...
The exported brief opens review items when there are no confirmed findings. It always includes a bounded **Local timeline (review-only)** selected deterministically from saved Shots, Pins and transcript passages; its wording is extractive, not an AI-generated conclusion. Processing status distinguishes unavailable transcription, speaker estimates, and AI analysis skipped without upload consent. The overview groups confirmed findings into highlights, decisions, actions and explicitly unresolved questions, with evidence links; it does not invent conclusions, owners or deadlines. Export actions link only existing, included files. If full-transcript export is enabled, the brief also offers a readable full transcript; only selected clips have playable video. Use **Rebuild local export** on an existing idle session to regenerate this local package without contacting a provider or requesting consent; it requires the saved transcript to be available.

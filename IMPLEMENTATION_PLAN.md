...
The user also requested recognizable project names after seeing Codex create a project called export. A desktop handoff now creates a recording-local codex-workspaces/ directory with a readable child name derived only from the exported context, UTC capture date and a stable recording identifier. It never opens the session root. The workspace contains a copied export/ projection and space for analysis notes. Reopening atomically refreshes only the reference export/, removing stale evidence and preserving notes outside it. Source files and destination directories are opened without following links; hard-linked source files and unowned workspaces are rejected. The prompt treats the recording label as metadata and asks the agent for a descriptive task title after reading the brief. Session deletion includes these derived workspaces and says so in its confirmation. No provider call is used to generate a name.

...
The user requested a working Open in Codex action in the transfer PR and explicitly chose support for both the desktop app and the Terminal CLI. Settings → AI stores this Mac's destination preference, with Codex app as the default. The desktop route opens a local draft with the validated export path and a fixed initial prompt through the documented codex scheme; it does not auto-submit. Menu, Overview and Recordings use the same routing. The legacy chatgpt enum values and diagnostic event names remain compatible.

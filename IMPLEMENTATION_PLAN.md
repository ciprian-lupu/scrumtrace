...
We do not claim that “uploading a zip to any AI understands video and images”. The default Tier 1 handoff receives `export/` only, never the live session root. **User-approved desktop-Codex exception (2026-09-15): Open archive in Codex adds an independent full-source snapshot to the same recording project**, after an explicit in-app confirmation. It includes the canonical manifest, available `archive/` files and corresponding `export/`. Both scopes preserve agent notes. Once added, the archive stays accessible in that project; an export-scoped prompt is not filesystem isolation. The draft is not auto-submitted, but analysis in Codex can transmit content to its model service. CLI and provider handoffs retain their existing export-only and upload-consent contracts.
...
## Approved addition — Mac-to-Mac transfer (2026-09-15)

The user approved importing recordings made on another Mac and explicitly opted into an optional complete transfer for reanalysis. This is a separate workflow from the agent handoff: C1–C5 remain in force and the normal measured agent zip stays at or below 35 MB.

The transfer package uses a versioned file index with byte counts and SHA-256 checksums. Import rejects unsafe paths, links, duplicate/index-mismatched files and checksum mismatches, stages privately, and installs only a complete validated session. It assigns a fresh local ID, keeps the original ID and source metadata, detects repeated imports and resets provider upload consent. It does not migrate credentials or machine permissions.

The private original manifest, transcript and timing form the comparison baseline. Imported sessions are marked in the library, and reanalysis creates a separate copy. Original media is required to transcribe again; an evidence-only pack must not imply that missing material can be recovered. New captures record the capture environment; legacy imports leave unknown capture details unknown. Source and receiver measurements must not be treated as directly comparable without matching inputs, models, settings and warm-up conditions.

A technical TRANSFER_REVIEW.md in export reports integrity, available materials, missing evidence, processing status and available measurements. It contains no captured text or machine name, participates in the existing pack budget, and does not count as hardware acceptance evidence.

The user additionally requested multi-selection before the transfer PR. The Recordings table supports native multi-selection, with single-recording actions disabled while several are selected. Batch export freezes the visible, readable selection, asks once for scope/private-data consent and one destination folder, and writes separate packages. The import panel accepts multiple packages/session folders. Each item commits independently; duplicate/existing items are skipped, failures do not stop later items, and cancellation preserves completed transfers while removing unfinished staging. A per-item result list describes partial completion.

## Approved handoff preference and repair (2026-09-15)

The user requested a working Open in Codex action in the transfer PR and explicitly chose support for both the desktop app and the Terminal CLI. Settings → AI stores this Mac's destination preference, with Codex app as the default. The desktop route opens a local draft with an export-only workspace and a prepared prompt through the documented codex scheme; it does not auto-submit. Menu, Overview and Recordings use the same routing. The legacy chatgpt enum values and diagnostic event names remain compatible.

Both destinations validate the export boundary. Explicitly included full transcripts are accepted using the export manifest's opt-in; omitted or unconsented transcripts, raw archive media and links remain blocked. Terminal errors are classified into fixed user-facing explanations without logging stderr. The user-invoked Terminal route is now a real consumer of Apple Events, so hardened builds include the Automation entitlement and the usage text explains that purpose. This does not change TCC grants or add a permission prompt to recording.

The user clarified the naming and archive workflow on 2026-09-15: one project per recording, two actions (Open export in Codex / Open archive in Codex), descriptive project names and simple task titles (Analyze export / Analyze archive). The first desktop open offers an editable name with the recorded context and UTC date as its default; no provider call generates names. A recording-local binding in codex-workspaces/ preserves the project identity across scope switches and metadata edits. PR #11 projects are adopted at their existing paths, preserving existing Codex tasks and notes; the documented link cannot rename their display labels. Export refresh atomically replaces export/; archive refresh atomically replaces source/ with the complete available source tree, even without an export. Notes outside these managed folders survive both. Source files and destination directories are opened without following links; hard-linked source files and unowned workspaces are rejected. Archive copies run off the UI actor while capture, transfers, processing and deletion are held busy. Session deletion includes workspaces and notes, and canonical transfers/reanalysis skip both current and legacy workspace folders. Existing separate private workspaces, if present, are left untouched rather than deleted or silently merged.
